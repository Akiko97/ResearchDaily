...
mul & div program, with usage of pointer:
...
div:
	pushq %rbp
	movq %rsp, %rbp
	xor %rax, %rax				; return value on error
	or %esi, %esi					; ensure %esi is not 0
	jz invalid						; if %esi is 0, end
	movl %edi, %eax
	movq %rdx, %r10
	cdq										; edx:eax
	idiv %esi
	movl %eax, (%r10)
	movl %edx, (%rcx)
	movq $0x1, %rax				; set return value 1
invalid:
	popq %rbp
	ret
```
...
* x86-32: `%eax`, `%edx` and `%ecx`, the rest go on stack
* x86-64: `%rdi`, `%rsi`, `%rdx`, `%rcx`, `%r8`, `%r9` (even for the functions with varargs), the rest go on the stack

(In Microsoft Visual C++, calling convention for x86-64 programs on Windows requires a calling function to pass the first four integer (or pointer) arguments using registers RCX, RDX, R8, and R9)

write a C + ASM program:
...
(Using the parameters in registers)

All the assembly code is writed in AT&T style.

The differences of AT&T style and Intel style:
...
The differences between x86-64 and x86-32:
...
![2022080901](./pic/2022080901.png)

* R8B~R15B is also called R8L~R15L (in Intel documents)

* the x86 instruction set imposes some notable restrictions on how general-purpose registers can be used:

  * imul: save the calculated integer product to RDX:RAX, EDX:EAX, DX:AX, or AX
  * idiv: requires the integer dividend to be loaded in RDX:RAX, EDX:EAX, DX:AX, or AX

  (the colon notation signifies that the final product is contained in two registers, with the first register holding the high-order bits)

  * x86 string instructions require that the addresses of the source and destination operands be placed in registers RSI and RDI, respectively. String instructions that include a repeat prefix must use RCX as the count register, while variable-bit shift and rotate instructions must load the count value into register CL.
  * processor uses register RSP to support stack-related operations such as function calls and returns, programs can also use the stack to pass function arguments and store temporary data. The RSP register always points to the stack's top most item.
  * register RBP is typically used as a base pointer to access data items that are stored on the stack.
...
CF: is set by the processor to signify an overflow condition when performing unsigned integer arithmetic; also used by some register rotate and shift instructions.

OF: signals that the result of a signed integer operation is too small or too large.

PF:  processor sets the parity flag to indicate whether the least-significant byte of an arithmetic, compare, or logical operation contains an even number of 1 bits (parity bits are used by some communication protocols to detect transmission errors).

SF & ZF: are set by arithmetic and logical instructions to signify a negative, zero, or positive result.

DF: Control bit. Program can set or reset the direction flag, which defines the auto increment direction (0 = low to high addresses, 1 = high to low addresses) of the RDI and RSI registers during execution of string instructions.

Other bits in the RFLAGS: **should never be modified by an application program**

(Reserved bits also should never be modified, and no assumptions should ever be made regarding the state of any reserved bit)

* RIP: contains the logical address of the next instruction to be executed. The RIP register is also used for displacement-based operand memory addressing.
...
* Memory Addressing:
  ...
  * IndexReg can be any general-purpose register except RSP.
  * Valid ScaleFactor include 2, 4, and 8.
  * Disp is a constant 8-bit, 16-bit, or 32-bit signed offset that's encoded within the instruction.
  ...
  * RIP-relative addressing allows the processor to reference global or static operands using a 32-bit displacement instead of a 64-bit displacement, which reduces required code space. It also facilitates position-independent code. One constraint of RIP-relative addressing is that the target operand must reside with a ± 2 GB address window of the value that’s contained in register RIP.

  (The calculation of a RIP-relative displacement value is automatically determined by the assembler during code generation)

## 2022/8/8

* x86 fundamental data type:

| Data Type       | Bits | Usages                                                     |
| --------------- | ---- | ---------------------------------------------------------- |
...
* Memory-aligned: the data type whose address is evenly divisible by its size in bytes.
* All multi-byte fundamental data types should be properly aligned whenever possible in order to avoid potential performance penalties that can occur when the processor accesses misaligned data.
* C/C++ type:

| type                        | <stdint.h> / <cstdint> | bits |
| --------------------------- | ---------------------- | ---- |
| char                        | int8_t                 | 8    |
| short                       | int16_t                | 16   |
| int, long                   | int32_t                | 32   |
...
| unsigned short              | uint16_t               | 16   |
| unsigned int, unsigned long | uint32_t               | 32   |
| unsigned long long          | uint64_t               | 64   |
| float                       | N/A                    | 32   |
| double                      | N/A                    | 64   |
...
This conclusion is only the results and analysis obtained from the data in this chapter, and is not intended to write high-performance programs like OpenBLAS.
